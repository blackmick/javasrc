...
CREATE TABLE tbl_asset_plan (
  ...
  `name`          VARCHAR(45) NOT NULL DEFAULT ''
  COMMENT '资产管理计划名称',
  `contract`      VARCHAR(64) NOT NULL DEFAULT '0'
  COMMENT '资产管理计划合同号',
...
CREATE TABLE tbl_asset_detail (
  ...
  COMMENT '分期期数',
  `payment_periods`      INT(11)        NOT NULL DEFAULT '1'
  COMMENT '分期计划',
  `account_type`         VARCHAR(32)    NOT NULL DEFAULT ''
  COMMENT '账户类型:',
  `repay_duration`       DATETIME       NOT NULL
  COMMENT '还款期限',
  `recevable_amount`     DECIMAL(10, 2)          DEFAULT '0.00'
  COMMENT '应还金额',
  `repaid`               DECIMAL(10, 2) NOT NULL DEFAULT '0.00'
  COMMENT '已还金额',
  `unrepaid`             DECIMAL(10, 2) NOT NULL DEFAULT '0.00'
  COMMENT '未还金额',
  `amount_for_period`    DECIMAL(10, 2)          DEFAULT '0.00'
  COMMENT '期供金额',
  `principle_for_period` DECIMAL(10, 2)          DEFAULT '0.00'
  COMMENT '期供本金',
  `repaid_principle`     DECIMAL(10, 2)          DEFAULT '0.00'
  COMMENT '已还本金',
  `cost_for_period`      DECIMAL(10, 2)          DEFAULT '0.00'
  COMMENT '期供费用',
  `repaid_cost`          DECIMAL(10, 2)          DEFAULT '0.00'
  COMMENT '已还费用',
  `interest_for_period`  DECIMAL(10, 2)          DEFAULT '0.00'
  COMMENT '期供利息',
  `repaid_interest`      DECIMAL(10, 2)          DEFAULT '0.00'
  COMMENT '已还利息',
  `fee`                  DECIMAL(10, 2)          DEFAULT '0.00'
  COMMENT '罚息',
  `repaid_fee`           DECIMAL(10, 2)          DEFAULT '0.00'
  COMMENT '已还罚息',
  `compound_interest`    DECIMAL(10, 2)          DEFAULT '0.00'
  COMMENT '复利',
  ...
  COMMENT '贷款利率(月)',
  `loan_duration`        DECIMAL(10, 2) NOT NULL
  COMMENT '贷款期限',
  ...
  COMMENT '贷款余额',
  `insurance_type`       INT            NOT NULL DEFAULT '0'
  COMMENT '担保类型',
  `over_due`             TINYINT(4)     NOT NULL DEFAULT '0'
  COMMENT '逾期标记',
  `over_due_time`        INT(11)        NOT NULL DEFAULT '0'
  COMMENT '累计逾期次数',
  `over_due_day`         INT(11)        NOT NULL DEFAULT '0'
  COMMENT '累计逾期天数',
  `max_over_due_day`     INT(11)        NOT NULL DEFAULT '0'
  COMMENT '历史最高逾期天数',
  `history_max_amount`   DECIMAL(10, 2) NOT NULL DEFAULT '0.00'
  COMMENT '历史最高金额',
  `over_due_principle`   DECIMAL(10, 2) NOT NULL DEFAULT '0.00'
  COMMENT '逾期本金金额',
  `over_due_interest`    DECIMAL(10, 2) NOT NULL DEFAULT '0.00'
  COMMENT '逾期利息金额',
  `account_age`          INT            NOT NULL DEFAULT '0'
  COMMENT '五级分类状态',
  `transfer_status`      TINYINT(4)     NOT NULL DEFAULT '0'
  COMMENT '转让状态:0-未转让;1-以转让',
  `asset_type`           TINYINT(4)     NOT NULL DEFAULT '0'
  COMMENT '资产状态:0-表内;1-表外',
  PRIMARY KEY (`id`)
)
  ENGINE = InnoDB
  DEFAULT CHARSET = utf8;
